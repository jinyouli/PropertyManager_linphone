...
-(void)setEvent:(NSDictionary*)event forTableView:(UITableView*)tableView withOtherName:(NSString *)otherName{
    if(event){
        ...
        NSString* content = event[@"message"];
...
//        NSString * dateS = [datef stringFromDate:date];
        self.labelDate.text = event[@"time"];
        
        LinphoneChatMessageState state = (LinphoneChatMessageState)[event[@"state"] integerValue];
        switch ([event[@"state"] integerValue]) {
