...
- (void)addScreenShot{
    ...
    if (WJYiOS7OrLater) {
        ...
        CGImageRef myImage = CGImageCreateWithImageInRect(viewImage.CGImage, CGRectMake(0, 20, 320, 460));
        originalImage = [UIImage imageWithCGImage:myImage];
...
- (void)setup{
    ...
    if (count == 1) {
        ...
        [self setButton:btn BackgroundWithButonType:(WJYAlertViewButtonType)[[btnDict.allKeys firstObject] integerValue]];
        [self addSubview:btn];
        btn.tag = 0;
