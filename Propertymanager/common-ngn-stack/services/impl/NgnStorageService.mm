...
#import "../../../PropertyManager/PropertyManager/Utility/Tool/UserManage/UserManagerTool.h"
...
+(BOOL) databaseCheckAndCopy:(NgnBaseService<INgnStorageService>*) service{
...
#endif
    sDataBasePath = [documentsDir stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.db",user.username]];
    ...
    if (![[NSUserDefaults standardUserDefaults]boolForKey:user.username]) {
        ...
        if(![fileManager copyItemAtPath:databasePathFromApp toPath:sDataBasePath error:&error]){
            ...
            [[NSUserDefaults standardUserDefaults]setBool:NO forKey:user.username];
            return NO;
        }
        else{
            [[NSUserDefaults standardUserDefaults]setBool:YES forKey:user.username];
        }
    }
...
-(BOOL) databaseOpen{
    [NgnStorageService databaseCheckAndCopy:self];
    sqlite3_open([sDataBasePath UTF8String], &self->database);
