...
public:
    ...
    int OnDialogEvent(const DialogEvent* _e){
        ...
        printf("_phrase ==========   %s",_phrase);
        // Not Found*[DOUBANGO INFO]: OnDialogEvent(Not Found, 2)
        const short _code = _e->getCode();
        short _sipCode;
        const SipSession* _session = _e->getBaseSession();
        const SipMessage* _sipMesssage = _e->getSipMessage();
        
        NSString * phraseStr = [NSString stringWithCString:_phrase encoding:NSUTF8StringEncoding];
        NSLog(@"phraseStr === %@",phraseStr);
        if ([phraseStr hasPrefix:@"Not Found"]) {
            //开头包含了字符串 对方占线
            [[NSNotificationCenter defaultCenter] postNotificationName:@"LineISBusy" object:nil];
            return -1;
        }
...
//  *****************  Connected   Audio/Video/MSRP(Chat, FileTransfer)   *******************************************
...
//  *****************  Terminated   Audio/Video/MSRP(Chat, FileTransfer)   *******************************************
                else if (((ngnSipSession = [NgnAVSession getSessionWithId:_sessionId]) != nil) || ((ngnSipSession = [NgnMsrpSession getSessionWithId: _sessionId]) != nil)){
                    eargs = [[NgnInviteEventArgs alloc]
                             initWithSessionId:_sessionId
...
-(BOOL)registerIdentity{
    ...
    if (sipStack == nil) {
        ...
        _mSipCallback;
        //        NSLog(@"sipPreferences.realm ==== %@",sipPreferences.realm);
        //        NSLog(@"sipStack ==== %@",sipStack);
