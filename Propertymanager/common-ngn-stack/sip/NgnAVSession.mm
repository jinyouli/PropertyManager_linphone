...
-(BOOL) hangUpCall{
    mConnectionState = CONN_STATE_CONNECTED;
    ...
    @synchronized (kSessions){
        ...
        if(avSession){
            if (sipMessage){
                ...
                NSLog(@"remoteStr === %@",remoteStr);
                
                if (remoteStr.length != 0) {
                    NSArray * componentArr = [remoteStr componentsSeparatedByString:@"\""];
                    if (componentArr.count > 1) {
                        NSString * Str1 = componentArr[1];
                        ...
                        for (int i = 0; i < codeArr.count; i ++) {
                            ...
                            [mStr appendFormat:[NSString stringWithFormat:@"%C",theChar]];
                            
                            NSLog(@"char === %@",[NSString stringWithFormat:@"%C",theChar]);
                        }
                        //设置disPlayName
                        displayName = mStr;
