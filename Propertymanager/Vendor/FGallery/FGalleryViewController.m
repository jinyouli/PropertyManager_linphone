...
#define kCaptionMarginLeft 25
#define kCaptionMarginRight 25
#define kDescPadding 3
#define kDescMarginLeft 25
#define kDescMarginRight 25
...
- (void)loadView
{
    ...
    _descContainer                      = [[UIView alloc] initWithFrame:CGRectZero];
    ...
    // setup caption
    _captionContainer.backgroundColor			= [UIColor colorWithWhite:.5 alpha:0];
    _captionContainer.hidden					= YES;
    _captionContainer.userInteractionEnabled	= NO;
    _captionContainer.exclusiveTouch			= YES;
    _caption.font								= [UIFont systemFontOfSize:16.0];
    _caption.textColor							= [UIColor whiteColor];
    _caption.backgroundColor					= [UIColor colorWithWhite:.8 alpha:0];
    _caption.textAlignment						= NSTextAlignmentCenter;
//    _caption.shadowColor						= [UIColor blackColor];
    _caption.shadowOffset						= CGSizeMake( 1, 1 );
    
    // setup desc
    _descContainer.backgroundColor              = [UIColor colorWithWhite:.5 alpha:0];
    _descContainer.hidden                       = YES;
    _descContainer.userInteractionEnabled       = NO;
    _descContainer.exclusiveTouch               = YES;
    _desc.font                                  = [UIFont systemFontOfSize:14.0];
    _desc.textColor                             = [UIColor whiteColor];
    _desc.backgroundColor                       = [UIColor colorWithWhite:0 alpha:0];
    _desc.textAlignment                         = NSTextAlignmentCenter;
//    _desc.shadowColor                           = [UIColor blackColor];
    _desc.shadowOffset                          = CGSizeMake( 1, 1 );
    ...
    [_container addSubview:_captionContainer];
    [_captionContainer addSubview:_caption];
    
    [_toolbar addSubview:_descContainer];
    [_descContainer addSubview:_desc];
...
{
    return [_photoLoaders objectForKey:[NSString stringWithFormat:@"%zi", _currentIndex]];
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self gotoImageByIndex:self.myImageIndex animated:NO];
}
...
- (void)removeImageAtIndex:(NSUInteger)index
{
	...
	FGalleryPhoto *photo = [_photoLoaders objectForKey:[NSString stringWithFormat:@"%zi",index]];
	...
	[_photoLoaders removeObjectForKey:[NSString stringWithFormat:@"%zi",index]];
...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    UIBarButtonItem *newBackButton = [[UIBarButtonItem alloc] initWithTitle:@"返回" style:UIBarButtonItemStylePlain target:self
...
- (void)updateCaption
{
	if([_photoSource numberOfPhotosForPhotoGallery:self] > 0 )
	{
        if([_photoSource respondsToSelector:@selector(photoGallery:captionForPhotoAtIndex:)])
        {
            ...
            if([caption length] > 0 )
            {
                float captionWidth = _container.frame.size.width - kCaptionPadding*2 - kCaptionMarginLeft - kCaptionMarginRight;
                ...
                _caption.numberOfLines = (numLines<=2)?numLines:2;
                _caption.text = caption;
                
                NSInteger containerHeight = height+kCaptionPadding*2;
                _captionContainer.frame = CGRectMake(kCaptionMarginLeft, 0, _container.frame.size.width, containerHeight );
                _caption.frame = CGRectMake(kCaptionPadding, kCaptionPadding, captionWidth, height );
                
                _captionContainer.hidden = NO;
                
                [self alignLabelWithTop:_caption];
            }
            else {
        ...
        if([_photoSource respondsToSelector:@selector(photoGallery:descriptionForPhotoAtIndex:)])
        {
            NSString *desc = [_photoSource photoGallery:self descriptionForPhotoAtIndex:_currentIndex];
            
            if([desc length] > 0 )
            {
                float descWidth = _container.frame.size.width-kDescPadding*2 - kDescMarginLeft - kDescMarginRight;
                CGSize textSize = [desc sizeWithFont:_desc.font];
                NSUInteger numLines = ceilf( textSize.width / descWidth );
                NSInteger height = ( textSize.height + kDescPadding ) * numLines;
                
                _desc.textAlignment = NSTextAlignmentLeft;
                _desc.numberOfLines = numLines;
                _desc.text = desc;
                
                NSInteger containerHeight = height+kDescPadding*2;
                _descContainer.frame = CGRectMake(kDescMarginLeft, -containerHeight, _container.frame.size.width, containerHeight );
                _desc.frame = CGRectMake(kDescPadding, kDescPadding, descWidth, height );
                
                // show desc bar
                _descContainer.hidden = NO;
            }
            else {
                // hide it if we don't have a desc.
                _descContainer.hidden = YES;
            }
...
{
    if (!_hideTitle){
        self.navigationItem.title =[NSString stringWithFormat:@"%ld %@ %i", _currentIndex+1, NSLocalizedString(@"/", @"") , [_photoSource numberOfPhotosForPhotoGallery:self]];
...
- (void)buildViews
{
	int count = [_photoSource numberOfPhotosForPhotoGallery:self];
...
- (void)preloadThumbnailImages
{
	...
	photo = [_photoLoaders objectForKey:[NSString stringWithFormat:@"%zi", index]];
	...
	{
		photo = [_photoLoaders objectForKey:[NSString stringWithFormat:@"%zi", curIndex]];
...
- (void)unloadFullsizeImageWithIndex:(NSUInteger)index
{
	if (index < [_photoViews count]) {		
		FGalleryPhoto *loader = [_photoLoaders objectForKey:[NSString stringWithFormat:@"%zi", index]];
...
- (FGalleryPhoto*)createGalleryPhotoForIndex:(NSUInteger)index
{
	...
	// store it
	[_photoLoaders setObject:photo forKey: [NSString stringWithFormat:@"%zi", index]];
